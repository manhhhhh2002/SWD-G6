...
function insertUser(user) {
  const sql =
    "INSERT INTO user (`name`, `email`, `password`, `is_verified`) VALUES (?)";
  return new Promise((resolve, reject) => {
    db.query(
      sql,
      [[user.name, user.email, user.password, user.is_verified]],
      (err, result) => {
        if (err) reject(err);
        resolve(result);
      }
...
function getUserByEmail(email) {
  const sql =
    "SELECT * FROM user LEFT JOIN setting ON user.role_id = setting.setting_id  WHERE user.email = ?";
  return new Promise((resolve, reject) => {
